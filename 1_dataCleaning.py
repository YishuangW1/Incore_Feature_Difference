...
def main():
    ...
    # 6. Optional: Drop Rows with Any Remaining NaNs
    logging.info("\nStep 6: Executing config.MISSING_VALUE_STRATEGY...")
    if getattr(config, 'MISSING_VALUE_STRATEGY', 'impute') == 'drop_rows':
        initial_rows = len(df)
        df.dropna(inplace=True)
        df.reset_index(drop=True, inplace=True)
        logging.info(f"  -> Dropped {initial_rows - len(df)} rows containing missing values. Current shape: {df.shape}")
    else:
        logging.info("  -> Deferring imputation to 2_dataPreprocessing (Train/Test Split isolation).")

    # 7. Add Random Feature
    logging.info("\nStep 7: Adding a random feature for modeling robustness checks...")
    df['random_feature'] = np.random.rand(len(df))

    # 7.5 Force Specific Numerical Variables to Categorical
    logging.info("\nStep 7.5: Converting EF Scale variables to Ordinal Categorical...")
    for col in ['EF_scale', 'tornado_EF_unc']:
